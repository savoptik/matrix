...
for i in 0..<matrix.count { // перебираем строки
    // проверяем, чтобы случайно не вывалится за пределы массива строк индекс строки обязательно должен быть нечётным так как элементы в массиве нумеруются с нулевого, а если будет к примеру 5 строк просто ничего не будет выполняться, так как от нуля это будет четвёртая строка, а парной для переставления у неё не будет.
